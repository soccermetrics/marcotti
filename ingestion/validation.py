...
class CountryIngest(BaseCSV):
    ...
    def parse_file(self, rows):
        ...
        for keys in rows:
            ...
            if self.session.query(Countries).filter_by(name=country_name).count() == 0:
...
class TimezoneIngest(BaseCSV):
    ...
    def parse_file(self, rows):
        ...
        for keys in rows:
            ...
            if self.session.query(Timezones).filter_by(name=timezone_region).count() == 0:
...
class PositionIngest(BaseCSV):
    ...
    def parse_file(self, rows):
        ...
        for keys in rows:
            ...
            if self.session.query(Positions).filter_by(name=position_name).count() == 0:
...
class SurfaceIngest(BaseCSV):
    ...
    def parse_file(self, rows):
        ...
        for keys in rows:
            ...
            if self.session.query(Surfaces).filter_by(description=surface_name).count() == 0:
